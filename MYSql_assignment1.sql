...
INSERT INTO emp (emp_id, first_name, last_name, salary, joining_date, department, gender, job_title  ) VALUES (1, 'Vikas',   'Kumar',    65000.00, '2021-07-01', 'IT',      'Male',   'Software Engineer'),
...
-- Upper and lower
SELECT UPPER(first_name) AS first_name_upper, LOWER(first_name) AS first_name_lower FROM emp;

-- Add full_name column and update
ALTER TABLE emp ADD COLUMN full_name VARCHAR(45);
UPDATE emp SET full_name = CONCAT(first_name, ' ', last_name);

-- Add 'Hello '
SELECT CONCAT('Hello ', first_name) AS hello_name FROM emp;
...
SELECT * FROM emp WHERE first_name = 'Malli';
SELECT * FROM emp WHERE first_name IN ('Malli','Meena','Anjali');
SELECT * FROM emp WHERE first_name NOT IN ('Malli','Meena','Anjali');
SELECT * FROM emp WHERE first_name LIKE 'v%';
SELECT * FROM emp WHERE first_name LIKE '%i';
SELECT * FROM emp WHERE first_name LIKE '%o%';
SELECT * FROM emp WHERE first_name REGEXP '^[m-v]';
SELECT * FROM emp WHERE first_name NOT REGEXP '^[m-v]';
SELECT * FROM emp WHERE first_name LIKE 'M____';
...
SELECT CONCAT(UPPER(SUBSTRING(first_name,1,1)), LOWER(SUBSTRING(first_name,2))) AS nice_name FROM emp;
...
SELECT GROUP_CONCAT(first_name SEPARATOR ', ') AS names_list FROM emp;
...
*/
SELECT RTRIM(first_name) AS right_trim, LTRIM(first_name) AS left_trim, TRIM(first_name) AS trimmed FROM emp;

#16. Query to check no.of records present in a table where employees having 50k salary.
SELECT COUNT(*) FROM emp WHERE salary>50000;

#17. Find the most recently hired employee in each department
SELECT * FROM emp s
WHERE joining_date = (SELECT MAX(joining_date) FROM emp WHERE department=s.department);

#-------Case When Then End Statement Queries
#1.Display first_name and gender as M/F.(if male then M, if Female then F)
SELECT first_name, CASE WHEN gender='Male' THEN 'M' WHEN gender='Female' THEN 'F' END AS gender_shorT FROM EMP;
...
SELECT first_name, salary,
  CASE WHEN salary < 50000 THEN 'Eligible for Raise' ELSE 'Not Eligible' END AS raise_eligibility FROM emp;
...
SELECT first_name, joining_date,
  CASE WHEN joining_date < '2022-01-01' THEN 'Experienced' ELSE 'New Hire' END AS status FROM EMP;
...
SELECT first_name,salary,seniority_level FROM EMP;
...
SELECT first_name, salary,
  CASE 
    ...
    ELSE 'Junior'
  END AS seniority_level FROM EMP;
...
SELECT first_name, joining_date,
  CASE WHEN joining_date >= '2024-01-01' THEN 'Recent Joiner' ELSE 'Long-Term Employee' END AS join_status FROM EMP;
...
SELECT first_name, salary, department,
  CASE WHEN department='IT' AND salary > 60000 THEN 'Promotion Eligible' ELSE 'Not Eligible' END AS promotion_eligibility FROM EMP;
...
SELECT first_name, salary,
  CASE WHEN salary < 50000 THEN 'Eligible for Overtime Pay' ELSE 'Not Eligible' END AS overtime_eligibility FROM EMP;
...
SELECT first_name, department, salary,
  ...
  END AS job_title_corrected FROM EMP;
...
SELECT department, SUM(salary) AS total_salary FROM emp GROUP BY department ORDER BY total_salary;
...
SELECT project_name FROM project_details GROUP BY project_name HAVING COUNT(emp_id_no) > 1;
...
SELECT department, (SELECT COUNT(DISTINCT department) FROM emp) AS total_departments, SUM(salary) FROM EMP GROUP BY department;
...
SELECT department, ROUND(AVG(salary),2) AS avg_dept_salary, MAX(salary) AS max_dept_salary FROM emp GROUP BY department ORDER BY avg_dept_salary;
...
-- 'Small')
 SELECT department, COUNT(emp_id) AS total_employees,
   CASE 
     ...
     ELSE 'Small'
   END AS dept_size FROM EMP GROUP BY department;
...
SELECT department, ROUND(AVG(salary),2) AS avg_dept_salary,
  ...
  ELSE 'Low Pay'
  END AS pay_level FROM EMP GROUP BY department;
...
SELECT department,gender, COUNT(emp_id) FROM emp GROUP BY department,gender ORDER BY department,gender;
...
-- 'Low Hiring')
SELECT YEAR(joining_date) AS joined_year, COUNT(emp_id) AS total_emp,
  CASE
     WHEN COUNT(emp_id) > 5 THEN 'High Hiring'
     WHEN COUNT(emp_id) BETWEEN 3 AND 5 THEN 'Moderate Hiring'
     ELSE 'Low Hiring'
  END AS hiring_trend FROM EMP GROUP BY joined_year;
...
SELECT department,
 CASE WHEN MAX(salary) > 70000 THEN 'Senior Leadership' ELSE 'Mid-Level' END AS leadership_level FROM EMP GROUP BY department;
...
SELECT department, COUNT(emp_id) AS num_high_salary,
  CASE WHEN COUNT(emp_id) > 2 THEN 'High-Paying Team' ELSE 'Low Paying Team' END AS team_label
FROM emp WHERE salary > 60000 GROUP BY department;

##--------------Date time related queries
-- 1.Query to extract the below things from joining_date column. (Year, Month, Day, Current
-- Date)
SELECT YEAR(joining_date) AS year_joining, MONTH(joining_date) AS joining_month, DAY(joining_date) AS joining_day, CURDATE() AS today FROM EMP;
...
ALTER TABLE emp ADD COLUMN month_diff INT;
ALTER TABLE emp ADD COLUMN day_diff INT;
UPDATE emp SET day_diff = DATEDIFF(CURDATE(), joining_date);
UPDATE emp SET month_diff = TIMESTAMPDIFF(MONTH, joining_date, CURDATE());

#3.Get all employee details from the employee table whose joining year is 2020.
SELECT * FROM emp WHERE YEAR(joining_date)=2020;

#4.Get all employee details from the employee table whose joining month is Feb.
SELECT * FROM emp WHERE MONTH(joining_date)=2;

-- 5.Get all employee details from employee table whose joining date between "2021-01-01"
-- and "2021-12-01"
SELECT * FROM emp WHERE joining_date BETWEEN '2021-01-01' AND '2021-12-01';
...
FROM emp s
JOIN project_details p ON s.emp_id = p.emp_id_no ORDER BY s.first_name;
...
SELECT s.first_name, p.project_name
FROM emp s
LEFT JOIN project_details p ON s.emp_id = p.emp_id_no ORDER BY s.first_name;
...
LEFT JOIN emp s ON p.emp_id_no = s.emp_id ORDER BY s.first_name;
...
FROM emp s
RIGHT JOIN project_details p ON s.emp_id = p.emp_id_no
GROUP BY project_name
HAVING COUNT(s.emp_id)<1;

-- 6.Get the employee name and project name for employees who are assigned to more than
-- one project.
SELECT p.project_name, COUNT(s.emp_id) AS emp_on_project
FROM emp s
RIGHT JOIN project_details p ON s.emp_id = p.emp_id_no
...
SELECT p.*
FROM project_details p
LEFT JOIN emp s ON p.emp_id_no = s.emp_id
...
-- descending order.
SELECT *, ROW_NUMBER() OVER (PARTITION BY department ORDER BY salary DESC) AS rownum_in_dept FROM EMP;

#2.Rank employees within each department based on salary using RANK().
SELECT *, RANK() OVER (PARTITION BY department ORDER BY salary DESC) AS rank_in_dept FROM EMP;

#3.Rank employees within each department based on salary using DENSE_RANK().
SELECT *, DENSE_RANK() OVER (PARTITION BY department ORDER BY salary DESC) AS dense_rank_in_dept FROM EMP;
...
SELECT *, RANK() OVER (PARTITION BY department, job_title ORDER BY salary DESC) AS job_rank FROM EMP;
...
-- PERCENT_RANK().
SELECT * FROM (
    SELECT emp_id, department, salary, PERCENT_RANK() OVER (PARTITION BY department ORDER BY salary DESC) AS pct_rank
    FROM emp
)z WHERE pct_rank<=0.1;

-- 5.Assign row numbers to employees based on their joining year using PARTITION BY
-- YEAR(join_date)
SELECT *, ROW_NUMBER() OVER (PARTITION BY YEAR(joining_date) ORDER BY joining_date) AS row_in_year FROM EMP;

#6.Rank employees based on the number of projects handled within each department
 SELECT 
    s.emp_id, s.department, COUNT(DISTINCT p.project_id) AS project_count,
    DENSE_RANK() OVER (PARTITION BY s.department ORDER BY COUNT(DISTINCT p.project_id) DESC) AS project_rank
FROM emp s
LEFT JOIN project_details p ON s.emp_id = p.emp_id_no
GROUP BY s.emp_id, s.department;   
